...
export default function TradingPanel({ onOutput }: TradingPanelProps) {
  ...
  const [positionType, setPositionType] = useState<'range' | 'binary'>('range')

  // Range Position Strikes
  const [lowerStrike, setLowerStrike] = useState<string>('')
  const [isNegInf, setIsNegInf] = useState<boolean>(false)
  const [higherStrike, setHigherStrike] = useState<string>('')
  const [isPosInf, setIsPosInf] = useState<boolean>(false)

  // Binary Position Strikes & Side
  const [binaryStrike, setBinaryStrike] = useState<string>('')
  ...
  const [quotePreview, setQuotePreview] = useState<{ mintCost: string; redeemPayout: string } | null>(null)
  ...
  const [oracleInfoLoading, setOracleInfoLoading] = useState<boolean>(false)

  // On-Chain positions state
  const [onChainPositions, setOnChainPositions] = useState<any[]>([])
  const [onChainPositionsLoading, setOnChainPositionsLoading] = useState(false)
  const [positionsView, setPositionsView] = useState<'onchain' | 'indexer'>('onchain')
  ...
  const getParsedInputs = () => {
    ...
    if (positionType === 'range') {
      let lower = 0n
      if (!isNegInf) {
        if (!lowerStrike) throw new Error('Lower strike is required')
        lower = BigInt(Math.floor(parseFloat(lowerStrike) * 1_000_000_000))
      }

      let higher: bigint = CONFIG.POS_INF_STRIKE
      if (!isPosInf) {
        if (!higherStrike) throw new Error('Higher strike is required')
      ...
      if (!binaryStrike) throw new Error('Strike is required')
  ...
  const fetchPositions = async () => {
    ...
    try {
      const response = await fetch(
        `https://predict-server.testnet.mystenlabs.com/managers/${managerId}/positions/summary`
      )
      if (response.ok) {
        const data = await response.json()
  ...
  const fetchOnChainPositions = async () => {
    ...
    try {
      ...
      const fields = (managerObj.data?.content as any)?.fields
      if (!fields) {
        setOnChainPositions([])
        return
      }
  ...
  const getMatchingPosition = () => {
    try {
      ...
      if (positionType === 'range') {
        const { lower, higher } = inputs as { lower: bigint; higher: bigint }
        return onChainPositions.find(
          (pos) =>
            pos.type === 'range' &&
        ...
        const { strike, isUp } = inputs as { strike: bigint; isUp: boolean }
        return onChainPositions.find(
          (pos) =>
            pos.type === 'binary' &&
            pos.oracle_id === oracleId &&
  ...
  const handlePreviewQuote = async () => {
    if (!account || !oracleId || !oracleInfo || !oracleInfo.expiryMs) return
    ...
    try {
      ...
      if (positionType === 'range') {
        ...
        readGetRangeTradeAmounts(tx, oracleId, rangeKey, 1_000_000n) // Unit contract (10^6 units)
      } else {
        const { strike, isUp } = inputs as { strike: bigint; isUp: boolean }
        const marketKey = buildMarketKey(tx, oracleId, expiryBigInt, strike, isUp)
        readGetTradeAmounts(tx, oracleId, marketKey, 1_000_000n) // Unit contract (10^6 units)
      }
  ...
  const handleMint = async () => {
    if (!account || !managerId || !oracleId || !oracleInfo || !oracleInfo.expiryMs) return
    setLoading(true)
    try {
      const inputs = getParsedInputs()
      const tx = new Transaction()
      ...
      if (positionType === 'range') {
        const { lower, higher, qty } = inputs as { lower: bigint; higher: bigint; qty: bigint }
        const rangeKey = buildRangeKey(tx, oracleId, expiryBigInt, lower, higher)
        txMintRange(tx, managerId, oracleId, rangeKey, qty, coinType)
      } else {
        const { strike, isUp, qty } = inputs as { strike: bigint; isUp: boolean; qty: bigint }
        const marketKey = buildMarketKey(tx, oracleId, expiryBigInt, strike, isUp)
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h2 className="text-lg font-bold">Trading Operations</h2>
        <p className="text-xs text-gray-400">
          Mint (Buy) or Redeem (Sell) vertical range position contracts
        </p>
      </div>
      ...
      <div className="rounded-xl border border-gray-800 bg-gray-950 p-4 space-y-3">
        ...
        {oraclesList.length > 0 && (
          <div>
            ...
            <select
              ...
              className="w-full rounded-lg bg-gray-900 px-3 py-1.5 text-xs border border-gray-800 focus:outline-none text-gray-200"
            >
              <option value="">-- Choose an active/settled market --</option>
              {oraclesList.map((o, idx) => (
                <option key={o.oracle_id} value={o.oracle_id}>
                  {idx === 0 ? '🔥 [LATEST] ' : ''}{o.underlying_asset} | {o.status.toUpperCase()} | Exp: {new Date(o.expiry).toLocaleDateString()} ({o.oracle_id.slice(0, 10)}...)
                </option>
              ))}
            </select>
          </div>
        ...
        <div>
          ...
          <input
            type="text"
            value={pythSourceId}
            onChange={(e) => setPythSourceId(e.target.value.trim())}
            placeholder="0x..."
            className="w-full rounded-lg bg-gray-900 px-3 py-1.5 text-xs border border-gray-800 focus:outline-none"
          />
        </div>
        ...
        <div className="rounded-xl border border-gray-800 bg-gray-950 p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs">
            <div className="rounded-lg bg-gray-900/40 p-2.5 border border-gray-800/50">
              <div className="text-gray-400 mb-0.5 font-medium">Spot Price</div>
              <div className="text-sm font-bold text-gray-100">${oracleInfo.spotPrice}</div>
            </div>
            <div className="rounded-lg bg-gray-900/40 p-2.5 border border-gray-800/50">
              <div className="text-gray-400 mb-0.5 font-medium">BS Forward Price</div>
              <div className="text-sm font-bold text-gray-100">${oracleInfo.forwardPrice}</div>
            </div>
            <div className="rounded-lg bg-gray-900/40 p-2.5 border border-gray-800/50">
              <div className="text-gray-400 mb-0.5 font-medium">Expiry</div>
              <div className="text-[11px] font-semibold text-gray-200 truncate" title={oracleInfo.expiry}>
                {oracleInfo.expiry}
              </div>
            </div>
            <div className="rounded-lg bg-gray-900/40 p-2.5 border border-gray-800/50">
              <div className="text-gray-400 mb-0.5 font-medium">Settlement Price</div>
              <div className="text-sm font-bold text-purple-400">
                {oracleInfo.settlementPrice ? `$${oracleInfo.settlementPrice}` : 'Not Settled'}
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="rounded-xl border border-gray-800 bg-gray-900/40 p-4 space-y-4">
        {/* Toggle Range / Binary */}
        <div className="flex gap-2 border-b border-gray-800/60 pb-3">
          <button
            type="button"
            onClick={() => { setPositionType('range'); setQuotePreview(null); }}
            className={`px-3.5 py-1.5 text-xs font-semibold rounded-lg transition ${
              positionType === 'range' ? 'bg-blue-600 text-white' : 'bg-gray-950 text-gray-400 hover:bg-gray-900'
            }`}
          >
            Range Position (Strikes Band)
          </button>
          <button
            type="button"
            onClick={() => { setPositionType('binary'); setQuotePreview(null); }}
            className={`px-3.5 py-1.5 text-xs font-semibold rounded-lg transition ${
              positionType === 'binary' ? 'bg-blue-600 text-white' : 'bg-gray-950 text-gray-400 hover:bg-gray-900'
            }`}
          >
            Binary Position (Single Strike Up/Down)
          </button>
        </div>
        ...
        {positionType === 'range' ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <label className="block text-xs font-medium text-gray-400">Lower Strike</label>
                <label className="flex items-center gap-1 text-[11px] text-gray-400 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={isNegInf}
                    onChange={(e) => setIsNegInf(e.target.checked)}
              ...
              <input
                ...
                className="w-full rounded-lg bg-gray-950 px-3 py-2 text-sm border border-gray-800 disabled:opacity-50"
              />
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <label className="block text-xs font-medium text-gray-400">Higher Strike</label>
                <label className="flex items-center gap-1 text-[11px] text-gray-400 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={isPosInf}
                    onChange={(e) => setIsPosInf(e.target.checked)}
                    className="rounded border-gray-800 bg-gray-950 text-blue-600 focus:ring-0"
                  />
                  Pos Infinity (max)
                </label>
              ...
              <input
                type="number"
                value={higherStrike}
                onChange={(e) => setHigherStrike(e.target.value)}
                disabled={isPosInf}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {/* Single Strike */}
            <div className="space-y-2">
              <label className="block text-xs font-medium text-gray-400 text-left">Strike Price</label>
              <input
                type="number"
                value={binaryStrike}
                onChange={(e) => setBinaryStrike(e.target.value)}
                placeholder="e.g. 74000"
                className="w-full rounded-lg bg-gray-950 px-3 py-2 text-sm border border-gray-800"
              />
            </div>

            {/* Direction UP/DOWN */}
            <div className="space-y-2">
              <label className="block text-xs font-medium text-gray-400 text-left">Direction (Side)</label>
        ...
        <p className="text-[10px] text-gray-400 italic text-left">
          * Strike inputs are automatically scaled by 10^9 (FLOAT_SCALING) under the hood.
        </p>
      </div>

      {/* Quantity & Actions */}
      <div className="rounded-xl border border-gray-800 bg-gray-900/40 p-4 space-y-4">
        <h3 className="font-semibold text-sm text-gray-200">Contracts Quantity</h3>
        <div>
          <input
            type="number"
            value={quantity}
            ...
            className="w-full rounded-lg bg-gray-950 px-3 py-2 text-sm border border-gray-800 focus:outline-none"
          />
          <p className="mt-1 text-[10px] text-gray-400 italic">
            * Contract quantities are scaled by 10^6 (6 decimals). 1.0 contract = 1,000,000 units.
          </p>
        </div>

        {/* Pricing Preview */}
        {quotePreview && (
          <div className="rounded-lg border border-blue-900 bg-blue-950/40 p-3 text-xs flex justify-between">
            <div>
              <span className="text-gray-400">Est. Unit Mint Cost: </span>
              <span className="font-semibold text-blue-200">${quotePreview.mintCost} dUSDC</span>
            </div>
            <div>
              <span className="text-gray-400">Est. Unit Redeem Payout: </span>
              <span className="font-semibold text-green-400">${quotePreview.redeemPayout} dUSDC</span>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
          <button
            ...
            disabled={previewLoading || loading || !managerId}
            className="w-full rounded-lg bg-gray-800 px-4 py-2.5 text-xs font-semibold text-gray-200 hover:bg-gray-700 disabled:opacity-50"
          >
            {previewLoading ? '⏳ Loading...' : '🔬 Preview Price'}
          </button>
          
          <button
            onClick={handleMint}
            disabled={loading || previewLoading || !managerId}
            className="w-full rounded-lg bg-blue-600 px-4 py-2.5 text-xs font-semibold hover:bg-blue-700 disabled:opacity-50"
          >
          ...
          <button
            ...
            disabled={loading || previewLoading || !managerId || currentOwnedQty <= 0}
            className="w-full rounded-lg bg-red-600 px-4 py-2.5 text-xs font-semibold hover:bg-red-700 disabled:opacity-50"
          >
            {loading ? '⏳ Processing...' : currentOwnedQty > 0 ? `📉 Sell (Redeem) [Owned: ${currentOwnedQty.toFixed(2)}]` : '📉 Sell (Redeem) [No Position]'}
          </button>
        </div>
        <p className="text-[10px] text-gray-400 mt-3 text-center border-t border-gray-800/40 pt-2">
          💡 **How to trade:** Choose a Direction (**UP** or **DOWN**), enter a Strike and Quantity, then click **Buy (Mint)** to open the position. Click **Sell (Redeem)** only to close/settle a position you already own (otherwise the transaction will fail on-chain).
        </p>
      </div>
        ...
        <div className="rounded-xl border border-gray-800 bg-gray-900/40 p-4 space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-gray-800/60 pb-3">
            <div className="text-left">
              <h3 className="font-semibold text-sm text-gray-200">Active Positions</h3>
              <p className="text-[10px] text-gray-400">View or close your open vertical range option contracts</p>
            </div>
            
            <div className="flex items-center gap-2">
              <div className="flex rounded-lg bg-gray-950 p-0.5 border border-gray-800">
                <button
                  type="button"
                  onClick={() => setPositionsView('onchain')}
                  className={`px-2.5 py-1 text-[10px] font-bold rounded-md transition ${
                    positionsView === 'onchain' ? 'bg-blue-600 text-white' : 'text-gray-400 hover:text-gray-200'
                  }`}
                >
                  Real-time (On-chain)
                </button>
                <button
                  type="button"
                  onClick={() => setPositionsView('indexer')}
                  className={`px-2.5 py-1 text-[10px] font-bold rounded-md transition ${
                    positionsView === 'indexer' ? 'bg-blue-600 text-white' : 'text-gray-400 hover:text-gray-200'
                  }`}
                >
                  Indexer PnL
                </button>
              ...
              <button
                ...
                className="text-[10px] bg-gray-800 hover:bg-gray-700 text-blue-400 hover:text-blue-300 font-semibold px-2.5 py-1.5 rounded-lg border border-gray-700 disabled:opacity-50"
              >
                {positionsLoading || onChainPositionsLoading ? '⏳' : '🔄'}
              </button>
            </div>
          </div>

          {positionsView === 'onchain' ? (
            onChainPositionsLoading ? (
              <div className="text-xs text-gray-500 py-6 text-center">⏳ Loading on-chain positions...</div>
            ) : onChainPositions.length === 0 ? (
              <div className="text-xs text-gray-500 py-6 text-center">
                📭 No active positions found on-chain for this manager
              </div>
            ) : (
                ...
                <table className="w-full text-xs text-left text-gray-400">
                  <thead className="text-[10px] text-gray-500 uppercase bg-gray-900/50">
                    <tr>
                      ...
                      <th className="p-2.5">Oracle ID</th>
                      <th className="p-2.5">Strike / Range</th>
                      <th className="p-2.5 text-right">Quantity</th>
                      <th className="p-2.5 text-center">Action</th>
                    </tr>
                  </thead>
                  <tbody>
                    {onChainPositions.map((pos, idx) => (
                      <tr 
                        key={idx} 
                        className="border-b border-gray-800/50 hover:bg-gray-900/20 last:border-none cursor-pointer"
                        onClick={() => {
                      ...
                      >
                        <td className="p-2.5 capitalize font-medium">
                          <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${
                            pos.type === 'binary' ? 'bg-indigo-950 text-indigo-400 border border-indigo-900/55' : 'bg-amber-950 text-amber-400 border border-amber-900/55'
                          }`}>
                            {pos.type}
                          </span>
                        </td>
                        <td className="p-2.5 font-mono truncate max-w-[120px]" title={pos.oracle_id}>
                          {pos.oracle_id}
                        </td>
                        <td className="p-2.5">
                          {pos.type === 'binary' ? (
                            <span>
                              {pos.is_up ? '🟢 UP' : '🔴 DOWN'} @ {(Number(pos.strike) / 1e9).toFixed(2)}
                            </span>
                          ) : (
                            <span>
                              {pos.lower_strike === '0' ? '0' : (Number(pos.lower_strike) / 1e9).toFixed(2)} -{' '}
                              {pos.higher_strike.startsWith('184467') ? '∞' : (Number(pos.higher_strike) / 1e9).toFixed(2)}
                            </span>
                          )}
                        </td>
                        <td className="p-2.5 text-right font-mono text-gray-200 font-semibold">
                          {(Number(pos.quantity) / 1e6).toFixed(2)}
                        </td>
                        <td className="p-2.5 text-center text-blue-400 font-bold hover:text-blue-300">
                          ✏️ Use
                        </td>
                      </tr>
                    ))}
            ...
            positionsLoading ? (
              <div className="text-xs text-gray-500 py-6 text-center">⏳ Loading position summary...</div>
            ) : managerPositions.length === 0 ? (
              <div className="text-xs text-gray-500 py-6 text-center">📭 No active positions found in indexer</div>
            ) : (
              <div className="overflow-x-auto rounded-lg border border-gray-800 bg-gray-950">
                <table className="w-full text-xs text-left text-gray-400">
                  <thead className="text-[10px] text-gray-500 uppercase bg-gray-900/50">
                  ...
                  <tbody>
                    {managerPositions.map((pos, idx) => (
                      ...
                      >
                        <td className="p-2.5 font-mono truncate max-w-[120px]" title={pos.oracle_id}>{pos.oracle_id}</td>
                        <td className="p-2.5">
                          {pos.is_up !== undefined ? (
                            <span>
                              {pos.is_up ? '🟢 UP' : '🔴 DOWN'} @ {(Number(pos.strike) / 1e9).toFixed(2)}
                            ...
                            <span>
                              {pos.lower_strike === '0' ? '0' : (Number(pos.lower_strike || 0) / 1e9).toFixed(2)} -{' '}
                              {pos.higher_strike?.toString()?.startsWith('184467') ? '∞' : (Number(pos.higher_strike || 0) / 1e9).toFixed(2)}
                            </span>
                          )}
                        </td>
                        <td className="p-2.5 text-right">
                          {(Number(pos.open_quantity !== undefined ? pos.open_quantity : (pos.quantity || 0)) / 1e6).toFixed(2)}
                        </td>
                        <td className={`p-2.5 text-right font-semibold ${
                          (pos.unrealized_pnl !== undefined ? (Number(pos.unrealized_pnl) + Number(pos.realized_pnl || 0)) : Number(pos.pnl || 0)) >= 0 
                            ? 'text-green-400' 
                            : 'text-red-400'
                        }`}>
                          {((pos.unrealized_pnl !== undefined ? (Number(pos.unrealized_pnl) + Number(pos.realized_pnl || 0)) : Number(pos.pnl || 0)) / 1e6).toFixed(4)} dUSDC
                        </td>
                      </tr>
                    ))}
                  </tbody>
